...
| `testar-medias.cjs` | a média do período nos cinco blocos com gráfico: pontilhado próprio, só meses realizados, teto e projeção sem média, um mês não produz reta, o interruptor guarda o que foi desligado, e o balão da própria reta traz soma, quantidade e QUAIS meses sem engolir o clique do mês |
...
| `testar-farol-mensal.cjs` | o Farol 3 e o agrupamento Objetivos/Faróis: quatro linhas com só a projeção tracejada, realizado até o mês fechado, total = fixas + variáveis, e o clique abrindo os lançamentos em ordem decrescente |
| `testar-farol-reconhecimento.cjs` | o Farol 2: o bloco "por centro de custo" saiu da fila e passou para dentro dele, "Despesas por reconhecer" migrou para o Painel com o recorte de lá, e clicar num centro abre uma tela só — não duas empilhadas |
